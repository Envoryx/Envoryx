...
export function serviceLabel(kind: string, version?: string, variant?: string): string {
  ...
  const name: Record<string, string> = { php: "PHP", web: webNames[variant ?? ""] ?? i18n.t("Web server"), node: "Node.js", python: "Python", go: "Go", ruby: "Ruby", database: dbNames[variant ?? ""] ?? i18n.t("Database"), redis: "Redis", memcached: "Memcached", mailpit: "Mailpit", rabbitmq: "RabbitMQ", meilisearch: "Meilisearch", typesense: "Typesense", opensearch: "OpenSearch", "opensearch-dashboards": "OpenSearch Dashboards", ollama: "Ollama", storage: i18n.t("Object storage") };
  // An additional database: its name, then the engine ("analytics · PostgreSQL 18").
  const extraDB = kind.startsWith("db-") ? `${kind.slice(3)} · ${dbNames[variant ?? ""] ?? i18n.t("Database")}` : undefined;
  const base = extraDB ?? name[kind] ?? kind;
