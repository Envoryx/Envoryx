...
describe("NewProjectPage wizard", () => {
  ...
  it("walks through the steps, previews and creates", async () => {
      ...
      "POST /projects/preview": () => ({
        body: {
          preview: {
            ...
            containers: [
              { service: "php", name: "staqio-shimly-api-php", image: "php:8.4-fpm", ports: [], mounts: ["/mnt/user/development/shimly-api → /var/www/html"] },
              { service: "web", name: "staqio-shimly-api-web", image: "caddy:2-alpine", ports: ["20000 → 80/tcp"], mounts: [] },
            ],
            volumes: [],
            images: ["php:8.4-fpm", "caddy:2-alpine"],
            warnings: ["image php:8.4-fpm will be pulled on first start"],
          },
        },
      }),
    ...
    expect(preview).toMatchObject({ name: "Shimly API", path: "shimly-api", docroot: "public", php: { version: "8.3" }, env: [{ key: "APP_ENV", value: "local" }] });
