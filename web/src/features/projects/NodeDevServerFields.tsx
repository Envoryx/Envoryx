...
export function NodeDevServerFields({ value, onChange, idPrefix = "node" }: { value: DevServerForm; onChange: (v: DevServerForm) => void; idPrefix?: string }) {
  ...
  return (
    <div className="space-y-3">
      <Checkbox
        label="Run a dev server"
        description="The script becomes the container's main process (restarted automatically) and is reachable at <slug>-dev.<base domain> through the proxy plus a direct host port. Run npm install first (Actions)."
        checked={value.devServer}
        onChange={(e) => set({ devServer: e.target.checked })}
      />
      {value.devServer && (
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Framework preset" htmlFor={`${idPrefix}-preset`} hint="Decides how host/port are passed to the script.">
          ...
          <Field label="Package manager" htmlFor={`${idPrefix}-pm`}>
          ...
          <Field label="Script" htmlFor={`${idPrefix}-script`} hint="package.json script name">
            <Input id={`${idPrefix}-script`} value={value.script} onChange={(e) => set({ script: e.target.value })} placeholder="dev" spellCheck={false} />
          </Field>
          <Field label="Port inside the container" htmlFor={`${idPrefix}-port`} hint="Vite: 5173, Next.js: 3000">
