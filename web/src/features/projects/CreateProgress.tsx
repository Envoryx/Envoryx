...
/**
 * Live view of the create operation while the wizard waits for it: the server-side step
 * (image pull with percentage, template scaffolding, container start) and the time spent.
 */
export function CreateProgress({ slug }: { slug: string }) {
  const { t } = useTranslation();
  const ops = useOperations();
  const op = ops.data?.find((o) => o.action === "create" && o.projectSlug === slug && !o.finishedAt);
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg border border-accent-500/30 bg-accent-500/5 px-4 py-3 text-sm" role="status" data-testid="create-progress">
      ...
      <div className="min-w-0 flex-1">
        <p className="font-medium text-fg">{t("Creating the project…")}</p>
        ...
        <p className="mt-1 text-xs text-subtle">{t("You can leave this page – the project keeps being created and appears in the list when done.")}</p>
      </div>
    </div>
  );
}
