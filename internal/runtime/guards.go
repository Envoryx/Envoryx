...
// every Envoryx runtime image; host and port never come from user input (a fixed network
// alias and a dialect's port), so interpolating them into the line is safe. The database
// images open their port only once initialisation is done – they run the setup server on
// a socket or with networking off – so a connection is a real readiness signal.
func WaitForTCP(host string, port int, label string) string {
	return fmt.Sprintf(`until socat -u /dev/null TCP:%s:%d 2>/dev/null; do echo 'envoryx: waiting for %s at %s:%d'; sleep 2; done`, host, port, label, host, port)
}
