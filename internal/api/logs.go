...
func serviceKind(r *http.Request) (store.ServiceKind, error) {
	...
	case store.ServicePHP, store.ServiceWeb, store.ServiceDatabase, store.ServicePython, store.ServiceNode, store.ServiceRedis, store.ServiceMailpit, store.ServiceStorage:
		return k, nil
	default:
