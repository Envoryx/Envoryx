...
func (s *Server) projectOut(ctx context.Context, v project.View) projectOut {
	...
	case "go":
		...
		if svc := p.Service(store.ServiceGo); svc != nil && json.Unmarshal(svc.Config, &gcfg) == nil {
			directPort = gcfg.HostPort
		}
	case "ruby":
		var rcfg runtime.RubyConfig
		if svc := p.Service(store.ServiceRuby); svc != nil && json.Unmarshal(svc.Config, &rcfg) == nil {
			directPort = rcfg.HostPort
		}
	}
	if directPort > 0 {
		host := ""
...
func (s *Server) registerTools() {
	...
	mcp.AddTool(s.mcp, s.tool(auth.ScopeRead, readOnly("list_runtimes", "List runtimes", "Available runtimes (PHP, Node.js, Python, Go, Ruby, web servers), database engines, services, PHP extension keys and templates for create_project.")), s.listRuntimes)
	mcp.AddTool(s.mcp, s.tool(auth.ScopeAdmin, mutating("create_project", "Create project", "Create a new development environment (web server plus PHP, Python, Go, Ruby and/or Node.js, optional database, Redis, Memcached, Mailpit, RabbitMQ, Meilisearch, Typesense, OpenSearch, Ollama, object storage, git clone or template). Returns the project including its URL.", false)), s.createProject)
	...
	mcp.AddTool(s.mcp, s.tool(auth.ScopeRead, readOnly("get_logs", "Get logs", "Log lines of one project container (web, php, python, go, ruby, node, database, redis, memcached, mailpit, rabbitmq, meilisearch, typesense, opensearch, opensearch-dashboards, ollama, or db-<name> for an additional database), optionally limited to a time range, a search text or warnings/errors. Each line carries the level Envoryx guesses from its text.")), s.getLogs)
...
type templateOut struct {
	...
	// Runtime the template scaffolds for: php, node, python, go or ruby.
...
func (s *Server) listRuntimes(_ context.Context, _ *mcp.CallToolRequest, _ listProjectsIn) (*mcp.CallToolResult, listRuntimesOut, error) {
	...
	for _, t := range project.Templates() {
		out.Templates = append(out.Templates, templateOut{ID: t.ID, Name: t.Name, Description: t.Description, Runtime: t.Runtime, Node: t.Node, Python: t.Python, Go: t.Go, Ruby: t.Ruby, Docroot: t.Docroot, RequiresDatabase: t.RequiresDatabase, RecommendedDatabase: t.RecommendedDatabase, Notes: t.Notes})
	}
...
type createProjectIn struct {
	...
	Template           string            `json:"template,omitempty" jsonschema:"Scaffold an application: laravel, symfony, wordpress, drupal, typo3, shopware, craft (PHP; the CMS ones need a database and are installed afterwards with run_action: drush:site-install, typo3:setup, shopware:install, craft:install), vite, next, nuxt (Node.js; needs nodeVersion and phpVersion \"none\" for a Node-only project) django, flask, fastapi (Python; needs pythonVersion and phpVersion \"none\"), go, gin, echo (Go; needs goVersion and phpVersion \"none\") or rails, rails-api, sinatra (Ruby; needs rubyVersion and phpVersion \"none\"). See list_runtimes for details. Cannot be combined with gitUrl."`
	PHPVersion         string            `json:"phpVersion,omitempty" jsonschema:"PHP version such as 8.4 (default: the catalogue default). Use \"none\" for a Python, Go, Ruby, Node-only or static project."`
	...
	GoVersion          string            `json:"goVersion,omitempty" jsonschema:"Add a Go container with this version (e.g. 1.27): the project's runtime (server) or a tooling container (go build, go test, modules)."`
	GoServer           bool              `json:"goServer,omitempty" jsonschema:"Build and run goPackage as the project's main process (air rebuilds on changes in dev mode). Without PHP or a Python server it is reachable at the project URL. Requires goVersion."`
	GoPackage          string            `json:"goPackage,omitempty" jsonschema:"Main package to build, relative to the project: . (default) or e.g. ./cmd/server."`
	GoPort             int               `json:"goPort,omitempty" jsonschema:"Port the server listens on inside the container, read from $PORT (default 8080)."`
	GoMode             string            `json:"goMode,omitempty" jsonschema:"dev (default, air live reload) or production (one build, then the binary)."`
	RubyVersion        string            `json:"rubyVersion,omitempty" jsonschema:"Add a Ruby container with this version (e.g. 3.4): the project's runtime (server) or a tooling container (bundle, rake, rails)."`
	RubyServer         bool              `json:"rubyServer,omitempty" jsonschema:"Run the server of rubyPreset as the project's main process (bundle install runs first). Without PHP or a Python or Go server it is reachable at the project URL. Requires rubyVersion."`
	RubyPreset         string            `json:"rubyPreset,omitempty" jsonschema:"Server preset: rails (bin/rails server, Puma in production) or rack (Puma on config.ru: Sinatra, Roda, Hanami). Default rails."`
	RubyPort           int               `json:"rubyPort,omitempty" jsonschema:"Port the server listens on inside the container (default: the preset's port, 3000 for rails, 9292 for rack)."`
	RubyMode           string            `json:"rubyMode,omitempty" jsonschema:"dev (default, RAILS_ENV/RACK_ENV development) or production."`
...
func (s *Server) createProject(ctx context.Context, _ *mcp.CallToolRequest, in createProjectIn) (*mcp.CallToolResult, projectOut, error) {
	...
	if v := strings.TrimSpace(in.GoVersion); v != "" {
		req.Go = &project.GoRequest{Version: v, Config: runtime.GoConfig{
			Server: in.GoServer, Mode: strings.ToLower(strings.TrimSpace(in.GoMode)), Package: strings.TrimSpace(in.GoPackage), Port: in.GoPort,
		}}
	}
	if v := strings.TrimSpace(in.RubyVersion); v != "" {
		req.Ruby = &project.RubyRequest{Version: v, Config: runtime.RubyConfig{
			Server: in.RubyServer, Mode: strings.ToLower(strings.TrimSpace(in.RubyMode)), Preset: strings.ToLower(strings.TrimSpace(in.RubyPreset)), Port: in.RubyPort,
		}}
	}
...
type getLogsIn struct {
	Project string `json:"project" jsonschema:"Project id, slug or name"`
	Service string `json:"service,omitempty" jsonschema:"Container: web, php, python, go, ruby, node, database, redis, memcached, mailpit, rabbitmq, meilisearch, typesense, opensearch, opensearch-dashboards, ollama or storage (default: the application container (php, else python, else go, else ruby, else node), else web)"`
