...
func (s *Server) registerTools() {
	...
	mcp.AddTool(s.mcp, s.tool(auth.ScopeAdmin, mutating("create_project", "Create project", "Create a new development environment (web server plus PHP, Python and/or Node.js, optional database, Redis, Memcached, Mailpit, RabbitMQ, object storage, git clone or template). Returns the project including its URL.", false)), s.createProject)
	...
	mcp.AddTool(s.mcp, s.tool(auth.ScopeRead, readOnly("get_logs", "Get logs", "Recent log lines of one project container (web, php, python, node, database, redis, memcached, mailpit, rabbitmq).")), s.getLogs)
...
type getLogsIn struct {
	Project string `json:"project" jsonschema:"Project id, slug or name"`
	Service string `json:"service,omitempty" jsonschema:"Container: web, php, python, node, database, redis, memcached, mailpit, rabbitmq or storage (default: the application container (php, else python, else node), else web)"`
...
func (s *Server) getLogs(ctx context.Context, _ *mcp.CallToolRequest, in getLogsIn) (*mcp.CallToolResult, getLogsOut, error) {
	...
	switch kind {
	case store.ServiceWeb, store.ServicePHP, store.ServicePython, store.ServiceNode, store.ServiceDatabase, store.ServiceRedis, store.ServiceMemcached, store.ServiceMailpit, store.ServiceRabbitMQ, store.ServiceStorage:
	default:
