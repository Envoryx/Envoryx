...
func (m *Manager) update(ctx context.Context, id string, req UpdateRequest) (View, error) {
	...
	if req.PHP != nil {
		if err := m.applyPHPUpdate(ctx, proj, *req.PHP, changes); err != nil {
			return View{}, err
		}
	}
	if req.Env != nil {
	...
	if req.Go != nil {
		if err := m.applyGoUpdate(ctx, proj, *req.Go, changes); err != nil {
			return View{}, err
		}
	}
	if req.Ruby != nil {
	...
	if req.OpenSearch != nil {
		r, err := m.applyExtraUpdate(ctx, proj, store.ServiceOpenSearch, *req.OpenSearch, changes)
		if err != nil {
			return View{}, err
		}
		recreateApp = recreateApp || r
...
// applyPHPUpdate changes, adds or removes the PHP service. Callers hold the lock. Adding
// PHP makes it the application again (routing, FastCGI, starter conventions follow from
// the service list), so the SPA fallback of the static configuration is dropped; removing
// it takes the PHP container and the PHP workers' containers with it – their definitions
// stay and come back with the runtime. The web configuration is regenerated by update()
// and applied by the restart that follows.
func (m *Manager) applyPHPUpdate(ctx context.Context, p store.Project, upd PHPUpdate, changes map[string]any) error {
	svc := p.Service(store.ServicePHP)
	if !upd.Enabled {
		if svc == nil {
			return nil
		}
		...
		if err := m.store.Projects.DeleteService(ctx, p.ID, store.ServicePHP); err != nil {
			return err
		}
		changes["php"] = "removed"
		return nil
	}
	v, err := m.catalog.Resolve("php", upd.Version)
	if err != nil {
		return err
	}
	cfg := upd.Config
	...
	if svc == nil {
		if err := m.store.Projects.AddService(ctx, store.ProjectService{ProjectID: p.ID, Kind: store.ServicePHP, Variant: "php", Version: v.Version, Image: v.Image, Enabled: true, Position: 10, Config: raw}); err != nil {
			return err
		}
		if web := p.Service(store.ServiceWeb); web != nil {
			if wcfg, err := webServiceConfig(*web); err == nil && wcfg.SPAFallback {
				if err := m.store.Projects.UpdateServiceConfig(ctx, p.ID, store.ServiceWeb, web.Version, web.Image, []byte("{}")); err != nil {
					return err
				}
				changes["spaFallback"] = false
			}
		}
		changes["php"] = v.Version
		return nil
	}
	if err := m.store.Projects.UpdateServiceConfig(ctx, p.ID, store.ServicePHP, v.Version, v.Image, raw); err != nil {
		return err
	}
	changes["php"] = v.Version
	return nil
...
// applyGoUpdate changes, adds or removes the Go service. Callers hold the lock. Removing
// it takes the Go container and the Go workers' containers with it – their definitions
// stay and come back with the runtime.
func (m *Manager) applyGoUpdate(ctx context.Context, p store.Project, upd GoUpdate, changes map[string]any) error {
	svc := p.Service(store.ServiceGo)
	switch {
	case !upd.Enabled && svc == nil:
		return nil
	case !upd.Enabled:
		gone := map[string]bool{string(store.ServiceGo): true}
		for _, w := range p.Workers {
			if preset, ok := workerPreset(w.Preset); ok && preset.Runtime == WorkerRuntimeGo {
				gone[string(WorkerKind(w))] = true
			}
		}
		containers, err := m.engine.ListContainers(ctx, true, p.ID)
		...
		if err := m.store.Projects.DeleteService(ctx, p.ID, store.ServiceGo); err != nil {
			return err
		}
		changes["go"] = "removed"
		return nil
	default:
		v, err := m.catalog.Resolve("go", upd.Version)
		if err != nil {
			return err
		}
		cfg := upd.Config
		...
		if svc == nil {
			if err := m.store.Projects.AddService(ctx, store.ProjectService{ProjectID: p.ID, Kind: store.ServiceGo, Variant: "go", Version: v.Version, Image: v.Image, Enabled: true, Position: 12, Config: raw}); err != nil {
				return err
			}
			changes["go"] = v.Version
			return nil
		}
		if svc.Version != v.Version || string(svc.Config) != string(raw) {
			if err := m.store.Projects.UpdateServiceConfig(ctx, p.ID, store.ServiceGo, v.Version, v.Image, raw); err != nil {
				return err
			}
			changes["go"] = v.Version
			if string(svc.Config) != string(raw) {
				changes["goServer"] = cfg.Server
				// Command/ports are baked into the container: remove it so ensurePlan recreates it.
				containers, err := m.engine.ListContainers(ctx, true, p.ID)
				if err != nil {
					return err
...
func (m *Manager) delete(ctx context.Context, id string, opts DeleteOptions) error {
	...
	if err := m.store.Metrics.DeleteProject(ctx, id); err != nil {
