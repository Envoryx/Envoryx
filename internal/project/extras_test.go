...
func TestPostgresAndMySQLDialects(t *testing.T) {
	...
	db, _ := e.engine.Container("staqio-shop-pg-database")
	...
	php, _ := e.engine.Container("staqio-shop-pg-php")
	...
	my, _ := e.engine.Container("staqio-shop-my-database")
...
func TestRedisAndMailpitServices(t *testing.T) {
	...
	redis, _ := e.engine.Container("staqio-full-redis")
	mail, _ := e.engine.Container("staqio-full-mailpit")
	if redis.Spec.Image != "redis:8-alpine" || redis.Spec.Mounts[0].Source != "staqio-full-redis" || redis.Spec.Ports[0].ContainerPort != 6379 {
	...
	if got := strings.Join(e.engine.VolumeNames(), ","); got != "staqio-full-database,staqio-full-redis" {
		t.Fatalf("volumes: %s", got)
	}
	php, _ := e.engine.Container("staqio-full-php")
	...
	if got := strings.Join(e.engine.VolumeNames(), ","); got != "staqio-full-database" {
	...
	for _, c := range e.engine.Calls {
		if strings.HasPrefix(c, "volume-remove:") && !strings.HasSuffix(c, "staqio-full-redis") {
