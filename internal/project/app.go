...
// goConfig returns the configuration of a project's enabled Go service, if any.
func goConfig(p store.Project) (runtime.GoConfig, bool) {
	svc := p.Service(store.ServiceGo)
	if svc == nil || !svc.Enabled {
		return runtime.GoConfig{}, false
	}
...
// goServesApp reports whether the Go server is the project's application (no enabled
// PHP service, no Python server, GoConfig.Server). The config is returned normalised.
func goServesApp(p store.Project) (runtime.GoConfig, bool) {
	if hasPHP(p) {
		return runtime.GoConfig{}, false
	}
