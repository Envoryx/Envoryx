...
version + extensions, database, Redis, Memcached, Mailpit, RabbitMQ, object storage, Node, Python, git clone, env),
...
`database`, `redis`, `memcached`, `mailpit`, `rabbitmq`, `storage`, `worker:<id>`). `envoryx project
